...
proc main*(cArgs: seq[string]) =
  ...
  var prompt: string = ""
  var threads: cint = 4
  var threshold: float32 = 0.04  # matches auto-editor's audio edit default
  ...
  # Both a file and ":mic" feed the same engine (transcribe.run, whisper.cpp
  # direct): resample to 16k mono, gate on auto-editor's silence detection, and
  # transcribe each utterance on its own.
