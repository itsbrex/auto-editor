## Microphone device selection (macOS only, via CoreAudio).
##
## Capture itself is done by FFmpeg's avfoundation indev (see cmds/whisper.nim);
## this module only decides *which* device to hand it. avfoundation can't tell a
## USB mic from a virtual one, so we classify by CoreAudio transport type and
## return a device name that avfoundation matches against `localizedName`.

import std/strformat

when defined(macosx):
  {.passL: "-framework CoreAudio".}  # CoreFoundation is linked already (config.nims)
